...
CSV_PATH = "C:/Users/astei/Source/Repos/aneutronic-fusion/_python/nacre_ddn.csv"  # NACRE II (Table 4): T9, adopted, low, high

# Temperature grid (1–1000 keV)
T_keV = np.logspace(0, 3, 400)

# DD (n) energetics: D(d,n)3He — charged energy available for direct heating ~ 0.82 MeV carried by 3He
Q_CHARGED_J = 0.82 * nr.MEV_TO_J
...
def save_csvs(T_keV, curves, Pbrem):
    ...
    df.to_csv("ddn_curves_for_tikz.csv", index=False)
    ...
    df_reac.to_csv("ddn_reactivity_for_tikz.csv", index=False)

def main():
    rates = load_all_rates(CSV_PATH)
    curves, Pbrem = compute_curves(rates, T_keV)
    save_csvs(T_keV, curves, Pbrem)
    print("Wrote: ddn_curves_for_tikz.csv and ddn_reactivity_for_tikz.csv")
